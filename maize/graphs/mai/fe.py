...
class FEPMapping(Graph):
    ...
    def build(self) -> None:
        ...
        self.connect_all(
            ...
            # See https://github.com/azu-biopharmaceuticals-rd/maize/issues/45
            (copy_ref.out, cast(Input[Isomer], dock.inp_ref)),
        )
...
class _FEP(Graph, register=False):
    ...
    def build(self) -> None:
        ...
        if self.backend == "autodock":
            ...
            dock = self.add(GNINA)
            # See https://github.com/azu-biopharmaceuticals-rd/maize/issues/45
            self.connect(copy_ref.out, cast(Input[Isomer], dock.inp_ref))
            self.combine_parameters(load_prot.file, dock.receptor, name="protein")
        ...
        tagp.values.set({"penalty": np.nan})
