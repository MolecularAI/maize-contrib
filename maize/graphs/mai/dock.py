...
class GlideDocking(Graph):
    ...
    def build(self) -> None:
        ...
        self.combine_parameters(
            glide_instance.query_interval, ligprep_instance.query_interval, name="query_interval"
        )


class PADMS(Graph):
    """
    Dock a Peptide/molecule using Align,rigid Docking, Minimize and Score
    using Ligprep, Schrödinger Tug align and Glide. Returns both aligned and minimized poses
    """

    inp: Input[list[str] | list[IsomerCollection]]
    out: Output[list[IsomerCollection]]
    aligned_output: Output[list[IsomerCollection]]
    ...
    inp_grid: Input[Annotated[Path, Suffix("zip")]]
    precision: Parameter[Literal["SP", "XP", "HTVS"]]
    glide_keywords: Parameter[GlideConfigType]
    max_score: Parameter[float]

    # Reference file for alignment
    reference_file: Input[Annotated[Path, Suffix("mae", "sdf")]]

    # Glide contraint loader, optional
    constraints: FileParameter[Annotated[Path, Suffix("in")]]

    def build(self) -> None:
        ligprep_instance = self.add(Ligprep)
        ligprep_instance.keywords.set({"ma": 400})

        # alignment to reference
        reference_loader = self.add(File2Molecule)
        alignment_node = self.add(SchrodingerTugAlignement, "STA")

        # copy the aligned output
        aligned_copy = self.add(Copy[list[IsomerCollection]], name="aligned_copy")

        # dock in place. Peptide setting should not be harmful for SM
        glide_instance = self.add(Glide, name="docking")
        glide_instance.keywords.set({"PEPTIDE": True, "DOCKING_METHOD": "mininplace"})

        self.connect_all(
            (ligprep_instance.out, alignment_node.inp),
            (reference_loader.out, alignment_node.align_reference),
            (alignment_node.out, aligned_copy.inp),
            (aligned_copy.out, glide_instance.inp),
        )

        self.map_port(ligprep_instance.inp, name="inp")
        self.map_port(glide_instance.out, name="out")
        self.map_port(aligned_copy.out, name="aligned_output")

        self.map_port(reference_loader.inp, name="reference_file")
