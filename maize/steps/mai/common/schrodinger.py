...
def _restart_jobserver(tempdir: bool = True) -> CompletedProcess[bytes]:
    cmd = CommandRunner(raise_on_failure=False)
    cmd.run_only("jsc local-server-stop")
    if tempdir:
        # It is very important that this directory is globally accessible
        # so that all Schrodinger nodes can use the same jobserver
        server_dir = mkdtemp(prefix=f"maize-jobserver-{unique_id(32)}")
        cmd.run_only(f"jsc local-server-dir --set {server_dir}")

    res = cmd.run_only("jsc local-server-start")
...
class Schrodinger(Node, register=False):
    ...
    query_interval: Parameter[int] = Parameter(default=2)
    """
    ...
    def prepare(self) -> None:
        if not _job_server_maize() or not _job_server_running():
            _restart_jobserver(tempdir=self.job_server_temp.value)
        ...
        while not self.signal.is_set():
            ...
            match _query_schrodinger_job(jobid):
                ...
                case _SchrodingerJobStatus.SERVER | _SchrodingerJobStatus.STOPPED:
                    ...
                    res = _restart_jobserver(tempdir=self.job_server_temp.value)
