...
class Glide(Schrodinger):
    ...
    required_callables = Schrodinger.required_callables + ["glide"]
    ...
    def prepare(self) -> None:
        # Override the default 10s job check with 60s to relieve load on the jobserver
        if not self.query_interval.is_set:
            self.query_interval.set(60)
    ...
    def run(self) -> None:
        ...
        if self.constraints.is_set:
            constraints_config = GlideConfig.from_file(self.constraints.filepath)
            ...
            maize_options = {
                ...
                "USE_REF_LIGAND",
                "CORE_RESTRAIN",
                "SHAPE_REF_LIGAND_FILE",
                "SHAPE_RESTRAIN"
            }
            if not ref:  # here we cannot allow core settings to be read from constraint file
                maize_options.add("CORE_DEFINITION")
            elif "CORE_DEFINITION" in constraints_config.options:  # handle CORE_DEFINITION override
                if "CORE_DEFINITION" in self.keywords.value.keys() or self.core_definition.changed:
                    # in this case, user set CORE_DEFINITION and it should not be overridden with the constraints file
                    self.logger.warning(
                        "Option CORE_DEFINITION provided in config file and also set in Maize, will override with Maize option %s",
                        config["CORE_DEFINITION"],
                    )
                    maize_options.add("CORE_DEFINITION")
            elif "SHAPE_REF_LIGAND_FILE" in constraints_config.options:
                # we need to check if is a shape reference or a regular reference ligand
                # in the case of a shape reference, we must change from "REF_LIGAND_FILE" to "SHAPE_REF_LIGAND_FILE"
                self.logger.info(
                    "Using shape constraint in Glide"
                )
                if "REF_LIGAND_FILE" in  constraints_config.options:
                    self.logger.error(
                        "Both REF_LIGAND_FILE and SHAPE_REF_LIGAND_FILE provided in config file, only one can be set. Using shape restraint ONLY"
                    )
                extra_config.options["SHAPE_REF_LIGAND_FILE"] = extra_config.options["REF_LIGAND_FILE"]
                extra_config.options["SHAPE_RESTRAIN"] = True

                for kw in ["REF_LIGAND_FILE","CORE_DEFINITION","CORE_RESTRAIN","USE_REF_LIGAND"]:
                    extra_config.delete(kw)

            invalid_options = set(itertools.chain(maize_options, self.keywords.value.keys()))
            for option in invalid_options:
                if option in constraints_config.options:
                    self.logger.warning("Option '%s' conflicts with Maize, removing...", option)
                    constraints_config.options.pop(option)

            extra_config.update(constraints_config)

        extra_config.to_file(glide_inp_file)
        self.logger.debug("Prepared GLIDE input for %s molecules", len(mols))
        self.logger.debug(
            "Using the following options for Glide: %s", extra_config.format_options()
        )
        ...
        self.logger.debug(f"Glide pre output list is {len(mols)} elements long")
        ...
        if output.exists():
            ...
            mols = merge_libraries(
                mols, docked, moltag="m_molid", isotag="m_isoid", merge_conformers=True
            )
        else:
        ...
        for mol in mols:
            for iso in mol.molecules:
                ...
                if not iso.has_tag(self.GLIDE_SCORE_TAG):
                    self.logger.warning(
                        "Docking failed for %s, no '%s' tag",
                        iso.name or iso.inchi,
                        self.GLIDE_SCORE_TAG,
                    )
                    iso.add_score(self.GLIDE_SCORE_TAG, np.nan)
                    continue
        ...
        self.logger.debug(f"exiting Glide, output list is {len(mols)} elements long")
...
@pytest.fixture
def constraint_input_prec(shared_datadir: Path) -> Path:
    return shared_datadir / "example2.in"


@pytest.fixture
def constraint_input_bad(shared_datadir: Path) -> Path:
    return shared_datadir / "example3.in"
...
class TestSuiteGlide:
    ...
    @pytest.mark.needs_node("glide")
    def test_Glide_constraints_extra(
        self,
        temp_working_dir: Path,
        test_config: Config,
        example_smiles: list[str],
        grid_constraints: Path,
        constraint_input_prec: Path,
    ) -> None:
        rig = TestRig(Glide, config=test_config)
        inputs = [IsomerCollection.from_smiles(smi) for smi in example_smiles]
        for inp in inputs:
        ...
        res = rig.setup_run(
            inputs={"inp": [inputs], "inp_grid": [grid_constraints]},
            parameters={"constraints": constraint_input_prec, "n_jobs": 4},
        )
        mols = res["out"].get()

        with Path("glide.in").open("r") as conf:
            assert "HTVS" in conf.read()
    ...
    @pytest.mark.needs_node("glide")
    def test_Glide_constraints_bad(
        self,
        temp_working_dir: Path,
        test_config: Config,
        example_smiles: list[str],
        grid_constraints: Path,
        constraint_input_bad: Path,
    ) -> None:
        rig = TestRig(Glide, config=test_config)
        inputs = [IsomerCollection.from_smiles(smi) for smi in example_smiles]
        for inp in inputs:
        ...
        res = rig.setup_run(
            inputs={"inp": [inputs], "inp_grid": [grid_constraints]},
            parameters={"constraints": constraint_input_bad, "n_jobs": 4},
        )
        mols = res["out"].get()

        with Path("glide.in").open("r") as conf:
            assert "bad_file" not in conf.read()
