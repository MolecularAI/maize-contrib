...
def _pdbqt2pdb(
    pdbqt: Annotated[Path, Suffix("pdbqt")], docked: Annotated[Path, Suffix("pdbqt")]
    ...
    def _parse_adv_outputs(
        ...
        for i, (key, file) in enumerate(outdict.items()):
            ...
            isomer.update_conformers_from_sdf(
                file.with_suffix(".sdf"), score_parser=_adv_score_parser_meeko, sanitize=False
            )
            isomer.set_tag("origin", self.name)
...
class VinaFlex(_Vina):
    ...
    def _parse_flex_adv_outputs(
        ...
        for i, (key, file) in enumerate(outdict.items()):
            ...
            resout[key] = _pdbqt2pdb(receptor, docked=file)
            isomer.update_conformers_from_sdf(
