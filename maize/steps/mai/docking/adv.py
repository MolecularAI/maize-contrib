...
def _pdbqt2pdb(
    pdbqt: Annotated[Path, Suffix("pdbqt")],
    docked: Annotated[Path, Suffix("pdbqt")],
    lig_present: bool = True,
) -> Annotated[Path, Suffix("pdb")]:
    """Converts PDBQT output to PDB, using flexible receptor information"""
    from meeko import PDBQTReceptor

    rigid = PDBQTReceptor.from_pdbqt_filename(pdbqt.as_posix()).atoms()
    flex = PDBQTReceptor.from_pdbqt_filename(docked.as_posix()).atoms()
...
def _write_sd_string(
    pdbqt_mol: "PDBQTMolecule", only_cluster_leads: bool = False, keep_flexresidues: bool = True
) -> tuple[str, list[int]]:
    ...
    sio = StringIO()
    f = Chem.SDWriter(sio)
    mol_list = RDKitMolCreate.from_pdbqt_mol(pdbqt_mol, only_cluster_leads, keep_flexresidues)
...
class _Vina(Node, register=False):
    ...
    scoring: Parameter[Literal["ad4", "vina", "vinardo"]] = Parameter(default="vina")
    """Scoring function to use"""

    max_mc_evals: Parameter[int] = Parameter(default=0)
    """Maximum number of Monte Carlo evaluations"""

    min_rmsd: Parameter[float] = Parameter(default=1.0)
    """Minimum RMSD between poses"""

    energy_range: Parameter[float] = Parameter(default=3.0)
    """Maximum energy difference between worst and best pos"""

    grid_spacing: Parameter[float] = Parameter(default=0.375)
    """Grid spacing in Angstroms"""

    version: Parameter[float] = Parameter(default=1.2)
    """Version of Vina"""
    ...
    def _parse_adv_outputs(
        ...
        for i, (key, file) in enumerate(outdict.items()):
            ...
            isomer.update_conformers_from_sdf(
                file.with_suffix(".sdf"),
                score_parser=_adv_score_parser_meeko,
                sanitize=self.sanitize.value,
            )
            isomer.set_tag("origin", self.name)
...
class Vina(_Vina):
    ...
    required_packages = ["meeko"]
    """Requires a custom environment with ``meeko>=0.5`` installed"""
        ...
        for i, mol in enumerate(mols):
            ...
            for j, isomer in enumerate(mol.molecules):
                ...
                command = (
                    ...
                    f"--energy_range {self.energy_range.value} "
                    f"--center_x {x} --center_y {y} --center_z {z} "
                    f"--size_x {dx} --size_y {dy} --size_z {dz} "
                )

                if self.version.value >= 1.2:
                    command += (
                        f"--max_evals {self.max_mc_evals.value} --min_rmsd {self.min_rmsd.value} "
                    )
                    command += (
                        f"--spacing {self.grid_spacing.value} --scoring {self.scoring.value} "
                    )
    ...
    def _parse_flex_adv_outputs(
        ...
        for i, (key, file) in enumerate(outdict.items()):
            ...
            if not file.exists() or file.stat().st_size == 0:
                ...
                continue
            _adv2sdf(file, file.with_suffix(".sdf"))  # file: pdbqt -> sdf
            complexes[key] = file.with_suffix(".sdf")

            lig, _ = self._split_mol_sdf(
                file.with_suffix(".sdf")
            )  # split sdf into ligand and residues sdf
