...
from maize.utilities.chem.chem import Conformer, merge_libraries
...
class DivPickerFilter(Node):
    """
    Picks N diverse isomer collections form a list of `IsomerCollection`.
    ...
    max_output_length: Parameter[int] = Parameter(default=10)
    """Return *n* diverse isomer collections"""
    ...
    def run(self) -> None:
        ...
        def compute_fingerprints(mols: list[Mol]) -> list[ExplicitBitVect]:

            mg = GetMorganGenerator()
            fps = []
            for mol in mols:
                if mol is not None:
                    fps.append(mg.GetFingerprint(mol))
            return fps

        isomer_collection_list = self.inp.receive()

        if not isomer_collection_list:
            self.out.send(isomer_collection_list)

        # Filter indices where the corresponding IsomerCollection is not empty
        filtered_isomer_collections = [ic for ic in isomer_collection_list if ic.molecules]

        # Compute fingerprints for the first isomer of each IsomerCollection using the already available rkdit mols
        fps = compute_fingerprints(
            [Chem.MolFromSmiles(ic.to_smiles()[0]) for ic in filtered_isomer_collections]
        )

        # run the diversity picker
        picker = MaxMinPicker()
        picked_indices = picker.LazyBitVectorPick(fps, len(fps), self.max_output_length.value)

        # filter the isomer collection to the picked indices
        filtered_isomer_collections = [filtered_isomer_collections[i] for i in list(picked_indices)]

        self.out.send(filtered_isomer_collections)
...
class TestSuiteFilter:
    ...
    def test_DivPickerFilter(self, test_config: Config) -> None:
        ...
        rig = TestRig(DivPickerFilter, config=test_config)
        result = rig.setup_run(inputs={"inp": [mols]}, parameters={"max_output_length": 3})
        res = result["out"].get()
        assert res is not None
        assert len(res) == 3
