...
# Code written by Mikhail Kabeshov and Michele Assante, 2022.
...
ConfTag = TypedDict(
    "ConfTag", {"atoms": list[AtomEntry], "energy": float | str | None, "gradient": float | None}
)
local_scratch = "/scratch/gaussian_tmp/"
...
class Bond:
    ...
    def __init__(self, atom1: Atom, atom2: Atom, order: float) -> None:
...
class Structure:
    ...
    def gaussian_link1(
        ...
        others
            Extra options for gaussian calculation
        multiplicity
        ...
        f.write(f"{percentage}chk={full_path}.chk\n")
        ...
        if not len(self.gaussian_heavy_atoms()) == 0:
            f.write(
                "# "
                + self.functional(f"{functional}/genecp ")
                + " "
            ...
            f.write(
                "# "
                + self.functional(f"{functional}/{basis} ")
                + " guess=read "
                + self.xqc(multiplicity)
                + " geom=allcheck "
                + options
    ...
    def functional(self, high_layer: str, low_layer: str | None = None) -> str:
        """
    ...
    def export_g16_into_maize(
        ...
        basis_set
            basis set to use
        others
            Extra options for calculations
        ...
        model_solv = "" if mode == "GSH_temp" else "smd,"

        full_path = str(os.path.abspath(self.name))
        new_input = full_path + ".gjf"
        ...
        with open(new_input, "w") as g16_inp:
            ...
            g16_inp.write(f"{percentage}chk={full_path}.chk\n")
            ...
            if mode == "minimum":
                if not len(self.gaussian_heavy_atoms()) == 0:
                    route_section = (
                        "# opt "
                        + self.xqc(multiplicity)
                        + f" freq scrf=({model_solv}solvent="
                        + solvent
                        + ",dovacuum) "
                        + self.functional(f"{functional}/genecp {others}")
                    ...
                    route_section = (
                        ...
                        + self.xqc(multiplicity)
                        + " freq "
                        + self.functional(f"{functional}/{basis_set} {others}")
                        + f" Integral(Grid=UltraFineGrid) scrf=({model_solv}solvent="
                        + solvent
                        + ",dovacuum)\n"
                    )
                ...
                route_section = "# opt uff=(SoftFirst, Print) geom=connectivity\n"

            g16_inp.write(route_section)
            g16_inp.write("\n")
            g16_inp.write(f"{self.name}\n")
            g16_inp.write("\n")
            ...
            if mode == "mm":
                if connectivity:
                    g16_inp.write(connectivity)
                    g16_inp.write("\n")
                    g16_inp.write("AtRad Br 0.75\n")
                    g16_inp.write("AtRad Ni4+2 1.40\n")
                    g16_inp.write("HrmStr3 Ni4+2 * 2.72 0.0\n")
                    g16_inp.write("\n")

            if not len(self.gaussian_heavy_atoms()) == 0 and mode != "mm":
                light_atoms = ""
                for i in self.gaussian_light_atoms():
                    light_atoms += i + " "
                light_atoms += str("0")
                g16_inp.write(light_atoms + "\n")
                g16_inp.write(f"{basis_set}" + "\n")
                g16_inp.write("****" + "\n")
    ...
    def connectivity_by_distance(self) -> None:
        ...
        bonds: list[Bond] = []
        c_matrixs: list[tuple[int, int, float]] = []
        for i in self.atoms:
                    ...
                    if not label == "N/A":
                        if len(Connectivity[label]) == 1:
                            if i.distance(j) < Connectivity[label][0]:
                                bond = Bond(i, j, 1.0)
                                bonds.append(bond)
                                c_matrixs.append((i.number, j.number, 1.0))
    ...
    def print_xyz_block(self, removeHs: bool = False) -> str:
        """
        This functions prints a Structure object as a XYZ block.

        Returns
        -----
        str
            String containing the coordinates of the sturcture
        """
        if self.name is None:
            self.name = "default"
        ...
        name_short = str((os.path.basename(self.name)))
        if removeHs:
            number = len([atom for atom in self.atoms if atom.label != "H"])
    ...
    def structure_to_rdmol(self, removeHs: bool = False) -> Chem.rdchem.Mol:
        """
        Converts coordinates of a Structure object into an RDKIT mol object
        ...
        if removeHs:
            xyz_block = self.print_xyz_block(removeHs=True)
        else:
            xyz_block = self.print_xyz_block()

        rdmol = Chem.rdmolfiles.MolFromXYZBlock(xyz_block)
        return rdmol

    def structure_to_tag(self) -> ConfTag:
        atom_id = int(1)
        atoms_list: list[AtomEntry] = []
        conformer_energy = self.energy
        gradient = None
        for atom in self.atoms:
            atom_entry: AtomEntry = {
                "element": str(atom.label),
                "atom_id": atom_id,
                "coords": [
    ...
    def coordinates_to_nparray(self) -> NDArray[np.float64]:
        """
        Convert mol_dict of a structure to numpy array of coordinates

        Returns
        -------
        NDArray[np.float64]
            Numpy array with atomic coordinates of Structure object.

        """
        l_coords : list[list[float]] = []
        for ec in self.mol_dict:
            l_coords.append(ec.coords)

        return np.array(l_coords)
...
class g_Output:
        ...
        with open(self.filename, "r") as f:
            for line in f.readlines():
                if "correction to Enthalpy" in line:
                    line_split = line.split()
                    pos = line_split.index("Enthalpy=") + 1
                    enthalpy_correction = float(line_split[pos]) / 627.503
                if "Thermal correction to Gibbs Free Energy= " in line:
                    line_split = line.split()
                    pos = line_split.index("Energy=") + 1
                    gibbs_correction = line_split[pos]
    ...
    def normal_termination(self) -> bool:
        ...
        with open(self.filename, "r") as f:
            ...
            try:
                ...
                print("Index error spotted in Gaussian")
                check = False
        return check
    ...
    def error_check_gaussian(self) -> str:
        ...
        with open(self.filename) as f:
            ...
            error_portion = f.readlines()[-5:]
            flag = "Error termination"
            match_lines = any([flag in line and line != flag for line in error_portion])

            if not match_lines:
                return timeout

            if (
                "Error termination via Lnk1e in /usr/local/Cluster-Apps/gaussian/g09/l9999.exe"
                in error_portion
            ):
                return steps_msg

            elif (
                "Error termination via Lnk1e in /usr/local/Cluster-Apps/gaussian/g09/l1.exe"
                in error_portion
            ):
                return steps_msg

            elif (
                "Error termination via Lnk1e in /usr/local/Cluster-Apps/gaussian/g09/l103.exe"
                in error_portion
            ):
                return geom_msg

            elif (
                "Error termination via Lnk1e in /usr/local/Cluster-Apps/gaussian/g09/l502.exe"
                in error_portion
            ):
                return geom_msg
            else:
    ...
    * RDKIT MOL to Structure
    ...
    def molecule_from_json(json_tag: list[ConfTag], name_tag: str) -> Structure:
        ...
        for atm in json_tag:
            ...
            for diz in atm_json:
                coords = diz["coords"]
                atom = Atom(diz["element"], [float(coords[0]), float(coords[1]), float(coords[2])])
                atmlst.append(atom)
                atom.number = atmnumb
                if atom.label == "H":  # choosing atom type
                    atom.type = "light"
...
def detect_clashes(molecule: Structure) -> list[Bond] | list[None]:
    """
    Function to detect clashes in a structure object geometry.
    Clashes are defined as bond with a distance below fractions (0.75 and 0.9)
    of interatomic distances contained in the Connectivity dictionary
    for defining bond.

    Parameters
    ----------
    molecule
        Structure object of the molecule geometry to check.
    Returns
    -------
    list[Bond] | list[None]
        Returns list of bond in molecule detected as clashes.
        Returns empty list if no clashes are detected.
    """

    connectivity_list = molecule.connectivity
    clash_list = []
    for bond in connectivity_list:
        distance = bond.length
        bond_label = "N/A"
        bond_label1 = bond.atom1.label + bond.atom2.label
        bond_label2 = bond.atom2.label + bond.atom1.label
        if bond_label1 in Connectivity:
            bond_label = bond_label1
        elif bond_label2 in Connectivity:
            bond_label = bond_label2

        if bond_label != "N/A":
            if len(Connectivity[bond_label]) == 1:
                if distance < (0.75 * Connectivity[bond_label][0]):
                    clash_list.append(bond)

            elif len(Connectivity[bond_label]) == 4:
                if distance < (0.90 * min(Connectivity[bond_label])):
                    clash_list.append(bond)

            elif bond_label in Coordinative_bonds:
                if distance < (0.75 * Coordinative_bonds[bond_label]):
                    clash_list.append(bond)

    return clash_list


def check_refined(raw: Structure, refined: Structure) -> tuple[bool, int]:
    """
    Function to check if calculation have refined an initial raw structure
    containing clashes generated with ReactionControl templates.

    Parameters
    ----------
    raw
        Structure object of raw geometry.
    refined
        Structure object of refined geometry.

    Returns
    -------
    tuple[bool, int]
        Tuple composed by boolean describing if the geometry is refined (True if refined) and
        by integer corresponding to the number of clashes left in the geometry.
    """

    clashes_raw = detect_clashes(raw)
    clashes_refined = detect_clashes(refined)

    is_refined = False

    if clashes_raw:
        if len(clashes_refined) <= len(clashes_raw):
            is_refined = True

    else:
        if not clashes_refined:
            is_refined = True

    return is_refined, len(clashes_refined)


def check_collaps(mol: Structure, idx1: int, idx2: int) -> bool:
    """
    Checks if a bond is present between two atoms
    ...
    atom1 = mol.mol_dict[idx1 - 1].to_Atom()
    atom2 = mol.mol_dict[idx2 - 1].to_Atom()

    return atom1.check_connected(atom2)
