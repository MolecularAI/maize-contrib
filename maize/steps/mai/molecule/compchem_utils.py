...
scratch = '/projects/mai/projects/reactivity_preds/test_maize/local_scratch'
...
class Structure:
    ...
    def gaussian_link1(
        ...
        if not len(self.gaussian_heavy_atoms()) == 0:
            f.write(
                "# "
                + self.functional(str(functional + "/genecp"), others)
                + " "
            ...
            f.write(
                "# "
                + self.functional(str(functional + "/" + basis), others)
                + " guess=read "
                + self.xqc(multiplicity)
                + " geom=allcheck "
                + options
    ...
    def export_g16_into_maize(
        ...
        with open(new_input, "w") as g16_inp:
            ...
            if mode == "minimum":
                ...
                route_section = (
                    "# opt uff=(SoftFirst, Print) geom=connectivity\n"
                )
        ...
        with open(self.filename, "r") as f:
            for line in f.readlines():
                ...
                if "Sum of electronic and thermal Free Energies" in line:
                    line_split = line.split()
                    pos = line_split.index("Energies=") + 1
                    gibbs_energy = line_split[pos]
