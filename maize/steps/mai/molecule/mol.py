...
class SaveSingleLibrary(Node):
    ...
    def run(self) -> None:
        mols = self.inp.receive()
        tags = self.tags.value if self.tags.is_set else None
...
class TestSuiteMol:
    ...
    def test_SaveSingleLibrary(self, tmp_path: Path, test_config: Config) -> None:
    ...
    def test_SaveSingleLibrary_inc(self, tmp_path: Path, test_config: Config) -> None:
        rig = TestRig(SaveSingleLibrary, config=test_config)
        mols = [
            IsomerCollection.from_smiles("Nc1ncnc(c12)n(CCCC)c(n2)Cc3cccc(c3)OC"),
            IsomerCollection.from_smiles("Nc1ncnc(c12)n(CCCC)c(n2)Cc3ccc(OC)cc3"),
    ...
    def test_SaveIsomers(self, tmp_path: Path, test_config: Config) -> None:
    ...
    def test_CombineMolecules(self, test_config: Config) -> None:
    ...
    def test_ExtractReference(self, test_config: Config) -> None:
    ...
    def test_AggregateScores(self, test_config: Config) -> None:
        mols = [
            IsomerCollection.from_smiles("Nc1ncnc(c12)n(CCCC)c(n2)Cc3cccc(c3)OC"),
            IsomerCollection.from_smiles("Nc1ncnc(c12)n(CCCC)c(n2)Cc3ccc(OC)cc3"),
        ]
