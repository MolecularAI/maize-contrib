...
from maize.steps.mai.molecule.compchem_utils import Loader
    ...
    def run(self) -> None:
        ...
        for i, smi in enumerate(smiles):
            ...
            try:
                ...
                if self.embed.value:
                    mol.embed(self.n_conformers.value, addh=self.addh.value)
                elif self.addh.value:
...
class LoadSmilesAsIsomerCollection(Node):
    ...
    smiles_field_name: Parameter[str] = Parameter(default="smiles")
    ...
    def run(self) -> None:
        ...
        component = ReactionComponent(
            component=mol_name, smiles=smiles, parameters=get_chg_and_mult(smiles_rdmol)
        )
...
class SaveCSV(Node):
    ...
    format: Parameter[Literal["isomers", "collections", "livedesign"]] = Parameter(
        default="isomers"
    )
    """
...
class TestSuiteMol:
    ...
    def test_Smiles2Molecules_addh(self, smiles1: str, test_config: Config) -> None:
        rig = TestRig(Smiles2Molecules, config=test_config)
        smiles = [smiles1]
        res = rig.setup_run(
            inputs={"inp": [smiles]}, parameters={"n_conformers": 1, "n_isomers": 1, "addh": False}
        )
        raw = res["out"].get()
        assert raw is not None
        mol = raw[0]
        assert mol.n_isomers <= 1
        assert not mol.scored
        assert mol.molecules[0].n_conformers == 1
        assert mol.molecules[0].charge == 0
        assert mol.molecules[0].n_atoms == 23

    def test_Smiles2Molecules_noembed(self, smiles1: str, test_config: Config) -> None:
        rig = TestRig(Smiles2Molecules, config=test_config)
        smiles = [smiles1]
        res = rig.setup_run(
            inputs={"inp": [smiles]}, parameters={"n_conformers": 1, "n_isomers": 1, "embed": False}
        )
        raw = res["out"].get()
        assert raw is not None
        mol = raw[0]
        assert mol.n_isomers <= 1
        assert not mol.scored
        assert mol.molecules[0].n_conformers == 0
        assert mol.molecules[0].charge == 0
        assert mol.molecules[0].n_atoms == 44
    ...
    def test_SaveSingleLibrary(
        self, smiles1: str, smiles2: str, tmp_path: Path, test_config: Config
    ) -> None:
    ...
    def test_SaveSingleLibrary_inc(
        self, smiles1: str, smiles2: str, tmp_path: Path, test_config: Config
    ) -> None:
        rig = TestRig(SaveSingleLibrary, config=test_config)
        mols = [
