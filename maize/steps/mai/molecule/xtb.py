...
from maize.steps.mai.molecule.compchem_utils import Loader, atom_string, check_collaps
...
class Xtb(Node):
    ...
    label: Parameter[str] = Parameter(default="XTB_calculation")
    """Name of the Gaussian job to be used in the logging."""

    mode: Parameter[str] = Parameter(default="sqm")
    """Select method to use for calculation (only SQM for now)"""
        ...
        for i, mol_folder in enumerate(mol_outputs):
            ...
            for j, iso_dirname in enumerate(mol_folder):
                ...
                if self.mode.value == "sqm":
                    isomer_final_geometries: dict[int, list[ConfTag] | list[AtomEntry] | str] = {}
                    ...
                    for k, conf_name in enumerate(cast(list[Path], iso_dirname)):
                        ...
                        outfile = (
                            conf_name / f"{isomer.get_tag('logging_name')}_conf{k}_xtb_out.txt"
                        )
                        ...
                        if not conf_output.exists() and self.opt.value:
                            ...
                            isomer_final_geometries[k] = "FAILED"
                            continue
                        ...
                        if self.opt.value:
                            optimized_geometry = Loader(str(conf_output)).molecule_xyz()

                            if Xtb.check_imaginary(outfile):
                                self.logger.info(
                                    f"Found Imaginary frequency for conf-{k}. Storing distorted geometry for restart."
                                )
                                exit_code = 1
                                conf_free_energy = Xtb.get_free_energy(outfile)[0]
                                isomer_trajectories[k] = Xtb.get_trajectory(
                                    conf_name / "xtbopt.log"
                                )
                                try:
                                    isomer_final_geometries[k] = Xtb.get_trajectory(
                                        conf_name / "xtbhess.xyz"
                                    )
                                except:
                                    isomer_final_geometries[k] = Xtb.get_trajectory(
                                        conf_name / "xtbopt.xyz"
                                    )
                                ...
                                if check_collaps(
                                    ...
                                    self.logger.warning(
                                        f"XTB optimisation failed for {conformer}: Collapsed to Product"
                                    )
                                    isomer_final_geometries[k] = "Collapsed to product"
                                    conf_free_energy = 1000.0
                                    isomer_free_energies[k] = conf_free_energy
                                    ...
                                    isomer_gradients[k] = Xtb.get_final_energy(conf_name)[1]
                                    isomer_free_energies[k] = conf_free_energy
                                    isomer_free_energy_corrections[k] = Xtb.get_free_energy(
                                        outfile
                                    )[1]
                    ...
                    isomer.set_tag(
                        "isomer_free_energy_corrections", json.dumps(isomer_free_energy_corrections)
                    )
                ...
                if self.mode.value == "mm":
                    ...
                    if exit_code == 0:
                        ...
                        self.logger.warning(
                            "XTB GFN-FF pre-optimisation failed for '%s'",
                            isomer.get_tag("logging_name"),
                        )
                        isomer_gfnff_exit_codes[j] = exit_code
    ...
    def run(self) -> None:
        ...
        for i, mol in enumerate(mols):
            ...
            self.logger.info(f"XTB optimisation for molecule {i}: {logging_name}")

            for j, isomer in enumerate(mol.molecules):
                isomer.set_tag("logging_name", logging_name)
                self.logger.info(
                    f"{mol.molecules[0].get_tag('logging_name')}: XTB optimisation for isomer {j}: {isomer}"
                )
                ...
                if mode_calc == "sqm":
                    ...
                    for k, conformer in enumerate(isomer.conformers):
                        self.logger.info(
                            f"{mol.molecules[0].get_tag('logging_name')} - isomer {j}: XTB optimisation for conformer {k}"
                        )
                        conformer_tag_dict[k] = f"{i}_{j}_{k}"
                        conf_path = iso_path / f"conformer-{k}"
                        conf_path.mkdir()
                        res_paths.append(conf_path)
                        input_flname = f"{isomer.get_tag('logging_name')}_conf{k}_inp.xyz"
                    ...
                    input_flname = f"{isomer.get_tag('logging_name')}_MM_inp"
                    ...
                    if isomer.has_tag("constraints"):
                        ...
                        self.logger.info(
                            f"found constraint for isomer {j}. Geometry Constraints and Symmetry controls added to xcontrol file."
                        )
                        ...
                        self.logger.info(
                            f"No constraints for isomer {j}. Only symmetry controls added to xcontrol file."
                        )
