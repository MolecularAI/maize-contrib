...
class ReactionControl(Node):
    ...
    reaction: Parameter[Literal["NiCatCycle_CN", "NiCatCycle_CC",
                                "RadicalTransformations",
                                "ConditionalRadicalTransformations1",
                                "ConditionalRadicalTransformations2"]] = Parameter(default="NiCatCycle_CC")
    """Reaction template to use"""

    _cache: dict[str, float]

### FOR DEBUGGING
    debug_save_location: FileParameter[Path] = FileParameter()
...
### END OF DEBUGGING
    ...
    def get_generics(self, row: pd.Series) -> None:
        ...
        if self.reaction.value == 'NiCatCycle_CC' or self.reaction.value == 'NiCatCycle_CN':
        ...
        for i in range(len(calc.molecules)):
            ...
            if calc.molecules[i].get_tag("crest_exit_code") == 0:
                ...
                if len(calc.molecules[i].conformers) > 0:
                    ...
                    crest_status = False
                    message += f" {n_conf_removed} conformer(s) were removed for connectivity errors."
                else:
                    message += " However, all conformers were removed due to connectivity errors."
    ...
    def assess_xtb_calculations(calc: IsomerCollection) -> tuple[bool, str]:
        ...
        for i in range(len(calc.molecules)):
            ...
            isomer = calc.molecules[i]
            isomer_exit_codes = json.loads(cast(str, isomer.get_tag("XTB_exit_codes")))
            
            if 0 in isomer_exit_codes.values():
                
                
                message = "XTB finished correctly."
                isomer_rd_mol = isomer._molecule
                isomer_structure = Loader.molecule_from_rdkit(isomer_rd_mol)
                
                conf_geometries_tag = json.loads(cast(str, isomer.get_tag("XTB_geometries")))
                n_conf = len(conf_geometries_tag)
                connectivity_string = f"{isomer.name}\n"
                for idx in range(len(conf_geometries_tag)):
                    conf_json = conf_geometries_tag[str(idx)]
                    conf_structure = Loader.molecule_from_json(conf_json, f"conformer-{idx}")

                    if not check_connectivity(isomer_structure, conf_structure):
                        connectivity_string += " Double check connectivity please."
                        
                if len(conf_geometries_tag) > 0:
                    n_conf_removed = n_conf - len(conf_geometries_tag) 
                    xtb_status = False
                    message += f" {isomer.name} {n_conf_removed} conformer(s) were removed for connectivity errors."
                else:
                    message += f" However, all conformers were removed due to connectivity errors. \n See connectivity matrix:\n {connectivity_string}"
    ...
    def assess_g16_calculations(calc: IsomerCollection) ->  tuple[bool, str]:
        ...
        g16_status = True
        message = "G16 calculation failed."

        for i in range(len(calc.molecules)):
            isomer_exit_codes = json.loads(cast(str, calc.molecules[i].get_tag("g16_exit_codes")))
    ...
    def run(self) -> None:
        ...
        if hasattr(self, 'others'):
            reaction = selected_reaction(self.id, self.rsmi, self.others, logger=self.logger, debug_save_location = self.debug_save_location)
        else:
            reaction = selected_reaction(self.id, self.rsmi, logger=self.logger, debug_save_location = self.debug_save_location)
        ...
        for ic in final_out:
            self.logger.info(
                f"Molecular Weight of {ic.molecules[0].get_tag('component')} is {ic.molecules[0].get_tag('mw')} "
            )
        ...
        n = 0
        while crest_results and n < 3:
            crest_to_resub = []
            n += 1
            for crest_calc in crest_results:
                self.logger.info(f"The isomer has {len(crest_calc.molecules[0].conformers)} conformers")
                
                status, message = self.assess_crest_calculations(crest_calc)
                if status:
                    crest_to_resub.append(crest_calc)
                    self.logger.info(message)
                    self.logger.info(f"The isomer has to be resubmitted, {len(crest_calc.molecules[0].conformers)} conformers left.")
                    self.logger.info(f"{crest_calc} is resubmitted for iteration number {n}.")
                else:
                    crest_completed.append(crest_calc)
                    self.logger.info(message)
                    self.logger.info(f"CREST for {crest_calc} is completed at iteration number {n}")
                    self.logger.info(f"Correct calc, it has {len(crest_calc.molecules[0].conformers)} conformers")
            ...
            if crest_to_resub:
                ...
                crest_results = []
                self.logger.info("no calc to resubmit to CREST")

        self.logger.info(f"completed list {crest_completed}")
        self.out_xg16.send(crest_completed)

        xg16_results: list[IsomerCollection] = self.inp_xg16.receive()
        self.logger.info(f"received list {xg16_results[0].molecules[0].name}")
        xg16_completed: list[IsomerCollection] = []
        self.logger.info(f"{len(xg16_results)} to be submitted to XTB and G16 nodes")

        m = 0
        while xg16_results and m < 3:
            ...
            for xg16_calc in xg16_results:
                ...
                if status_xtb or status_g16:
                    conformers_tag = json.loads(cast(str, xg16_calc.molecules[0].get_tag("XTB_geometries")))
                    n_conformers = len(conformers_tag)
                 
                    self.logger.info(f"XTB: {message_xtb}\n G16: {message_g16}")
                    self.logger.info(f"The isomer has to be resubmitted, {n_conformers} conformers left.")
                    self.logger.info(f"{xg16_calc} is resubmitted for iteration number {m}.")
                    xg16_to_resub.append(xg16_calc)
                else:
                    xg16_completed.append(xg16_calc)
...
class TestSuiteReactionControl:
    ...
    def test_full_wf(
        ...
        correct.molecules[0].name = 'Correct Molecule'
        correct.molecules[0].set_tag("crest_exit_code", 0)
        correct.molecules[0].clear_conformers()
        correct.molecules[0].add_conformer(Conformer.from_rdmol(correct_conf1, parent=correct.molecules[0], renumber=False, sanitize=False))
        correct.molecules[0].set_tag("XTB_exit_codes", '{"0" : 0}')
        correct.molecules[0].set_tag("g16_exit_codes", '{"0" : 0}')
        correct.molecules[0].set_tag(
            "XTB_geometries", json.dumps({'0': [TestSuiteReactionControl.rdmol_to_jsontag(correct_conf1)]})
        )
        
        wrong.molecules[0].name = 'Wrong Molecule'
        wrong.molecules[0].set_tag("crest_exit_code", 0)
        wrong.molecules[0].clear_conformers()
        wrong.molecules[0].add_conformer(Conformer.from_rdmol(wrong_conf2, parent=wrong.molecules[0], renumber=False, sanitize=False))
        wrong.molecules[0].set_tag("XTB_exit_codes", '{"0" : 0}')
        wrong.molecules[0].set_tag("g16_exit_codes", '{"0" : 0}')
        wrong.molecules[0].set_tag(
              "XTB_geometries", json.dumps({'0': [TestSuiteReactionControl.rdmol_to_jsontag(wrong_conf2)]})
         )
        ...
        for mol in to_xg16:
            ...
            assert mol.molecules[0].name == "Correct Molecule"
