...
from rdkit.Chem import Mol, AllChem, rdFMCS, rdchem, rdGeometry, rdMolAlign, rdMolTransforms
...
try:
    from maize.utilities.utilities import timeout as _timeout
except ImportError:
...
def refresh_conformer_wrappers(iso) -> None:
    """
    Rebuild the conformer wrappers for an IsoMolecule.

    Iterates over all RDKit conformers in `iso._molecule`, creates a
    corresponding `Conformer` wrapper for each, and updates
    `iso._conformers` with the new list.
    """
...
def to_explicit_bitvect(array: list[bool]) -> ExplicitBitVect:
    """
    Converts an array of boolean or truthy values to an RDKit ExplicitBitVect.

    Parameters
    ----------
    array : list or iterable
        An array of boolean, string, or integer values representing bits.

    Returns
    -------
    rdkit.DataStructs.cDataStructs.ExplicitBitVect
        The corresponding ExplicitBitVect object.
    """
    bv = ExplicitBitVect(len(array))
...
def load_sdf_or_mae_library(
    ...
    """
    Loads an SDF or MAE library containing multiple molecules with potentially
    ...
    if input_suffix == ".sdf":
        ...
        log.info("reading from mae")
        backend = Chem.MaeMolSupplier
        file_handle = file.as_posix()
    elif input_suffix == ".maegz":
        log.info("reading from mae.gz")
        backend = Chem.MaeMolSupplier
...
load_sdf_library = load_sdf_or_mae_library  # legacy compatibility
...
class Isomer(_ScorableMixin):
    ...
    * Different protonation states
    ...
    def __setstate__(self, state: dict[str, Any]) -> None:
        conf_tags = state.pop("_conformer_tags", [])
        self.__dict__ = state
        self._init_conformers(conformer_tags=conf_tags)

    def __getstate__(self) -> dict[str, Any]:
        inst = self.__dict__.copy()
        inst["_conformer_tags"] = [conf.tags for conf in self.conformers]
        inst["_conformers"] = []
        return inst
    ...
    @classmethod
    def from_sdf_block(cls, sdf: str, remove_hydrogens: bool = False) -> Self:
        """
        ...
        """
        rd_mol = result_check(Chem.MolFromMolBlock)(sdf, removeHs=remove_hydrogens)
        return cls(rd_mol=rd_mol)

    @classmethod
    def from_pdb(cls, file: Path, sanitize: bool = True, remove_hydrogens: bool = False) -> Self:
        """
        Create a molecule from an PDB file.
        ...
        ------
        ChemistryException
            If there was an error parsing the PDB file.
        """
        try:
            rd_mol = Chem.MolFromPDBFile(
                file.as_posix(), sanitize=sanitize, removeHs=remove_hydrogens
            )
            if rd_mol is None:
                raise ChemistryException(f"PDB file '{file.name}' produced an empty molecule")
        except Exception as e:
            raise ChemistryException(f"Failed to read PDB file '{file.name}': {e}")

        return cls(rd_mol=rd_mol)

    @classmethod
    def from_pdbblock(cls, block_text: str, sanitize: bool = True) -> Self:
        """
        Create a molecule from a string in PDB format.
        ...
        ------
        ChemistryException
            If there was an error parsing the block file.
        """
        try:
            rd_mol = Chem.MolFromPDBBlock(block_text, sanitize=sanitize)
            if rd_mol is None:
                raise ChemistryException(f"PDB block '{block_text}' produced an empty molecule")
        except Exception as e:
            raise ChemistryException(f"Failed to read PDB block '{block_text}': {e}")
    ...
    def addh(self, add_coords: bool = False) -> None:
        """Adds hydrogens to the molecule"""
        self._molecule = Chem.AddHs(self._molecule, addCoords=add_coords)
    ...
    def embed(self, n_conformers: int = 1, addh: bool = True) -> None:
        """
    ...
    def _init_conformers(self, conformer_tags: list[dict[str, Any]] | None = None) -> None:
        ...
        if conformer_tags is not None:
            for tags, conf in zip(conformer_tags, self._conformers):
                for tag, value in tags.items():
                    conf.set_tag(tag, value)
...
class IsomerCollection:
    ...
    def scores(self) -> dict[str, float]:
        ...
        if not self.n_isomers:
            # this is a fail-safe for empty isomer collections
            # it needs to apply the same as for single isomers from ScoreMixin
            # there is no need to aggregate anything here because there are no isomers
            iso_collection_scores = {}
            for int_name in self._score_tags:
                _, _, name = int_name.split("__")
                iso_collection_scores[name] = float(cast(float, self.get_tag(int_name, np.nan)))
            return iso_collection_scores
    ...
    @property
    def inchi(self) -> str:
        """Returns the main and /q layer part of the InChIKey for the molecule"""
        if self.n_isomers > 0:
            return self.molecules[0].inchi.split("-")[0]
        return ""

    @property
    def complete_inchi_key(self) -> str:
        """Returns the complete InChIKey for the molecule"""
        if self.n_isomers > 0:
            return self.molecules[0].inchi
        return ""
    ...
    def embed(self, n_conformers: int = 1, addh: bool = True) -> None:
        ...
        for mol in self.molecules:
            mol.embed(n_conformers, addh=addh)
